from traders_backtesting.trader_mid_05 import Trader
...
tutorial_round = ["AMETHYSTS", "STARFRUIT"]
...
tutorial_round_pst = ["AMETHYSTS", "STARFRUIT"]

SYMBOLS_BY_ROUND_POSITIONABLE = {
    0: tutorial_round_pst,
}
...
def simulate_alternative(
    ...
    df_prices = pd.read_csv(activity_path, sep=';')
    ...
    create_log_file(log_path, states, profits_by_symbol, balance_by_symbol, trader)
...
def trades_position_pnl_run(
        ...
        for time, state in states.items():
            ...
            orders, converstions, traderData = trader.run(state)
...
def create_log_file(log_path, states: dict[int, TradingState], profits_by_symbol: dict[int, dict[str, float]], balance_by_symbol: dict[int, dict[str, float]], trader: Trader):
    max_time = max(list(states.keys()))
...
if __name__ == "__main__":
    ...
    round = 0
    activity_path = "analysis/backtestData/tutorial_activity.csv"
    trades_path = "analysis/backtestData/tutorial_trades.csv"
    log_path = "analysis/backtestLogs/tutorial_log.csv"
    simulate_alternative(activity_path, trades_path, log_path, round, trader, max_time, names, halfway, monkeys)
