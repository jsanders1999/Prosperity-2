    ...
    def run(self, state: TradingState):
        ...
        for product in state.order_depths:
            ...
            if product == "STARFRUIT":
                ...
                orders, conversions, traderDataDict, loggingDataDict = trade_orchids(order_depth, traderDataDict, loggingDataDict, product, orders, position, conversions, state.observations)
...
def trade_orchids(order_depth, traderDataDict, loggingDataDict, product, orders, position, conversions, observations):    
    ...
    foreign_bid = observations.plainValueObservations["ORCHIDS"].bidPrice
    foreign_ask = observations.plainValueObservations["ORCHIDS"].askPrice
    ...
    if export_price > acceptable_price:
        #export

    #TODO: put constants in an external dictionary
    position_limit = (-100,0) #LEVI: Hier kun je de positie limiet aanpassen die je hanteert
    bound = 1 #LEVI: Hier kun je aanpassen voor hoeveel goedkoper je inkoopt en duurder je verkoopt

    orders, conversions = trade_constant_conversions(product, orders, conversions, position, acceptable_price, position_limit, bound)
...
def trade_constant_conversions(product, orders, conversions, position, acceptable_price, position_limit, bound):
    buy_amount = 0#position_limit[1] - position
    ...
    conversions += -(position) #LEVI: Het magische conversions getal, geen idee hoe het werkt. Doet iets met imports en exports ofzo. Miss snap jij de wiki erover wel?
