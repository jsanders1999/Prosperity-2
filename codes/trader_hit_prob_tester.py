    ...
    def run(self, state: TradingState):
        ...
        for product in state.order_depths:
            ...
            if product == "STARFRUIT":
                ...
                orders, traderDataDict, loggingDataDict = trade_gift_baskets(order_depth, traderDataDict, loggingDataDict, product, orders, position)
...
def trade_chocolate(order_depth, traderDataDict, loggingDataDict, product, orders, position):
    ...
    orders = trade_constant_deltas(product, orders, position, acceptable_price, position_limit, delta_p, delta_n)
...
def trade_gift_baskets(order_depth, traderDataDict, loggingDataDict, product, orders, position):
    weighted_mid_price = calc_weighted_mid_price(order_depth.buy_orders, order_depth.sell_orders)
    acceptable_price = weighted_mid_price
...
def trade_orchids(order_depth, traderDataDict, loggingDataDict, product, orders, position, conversions, observations, timestamp):    
    ...
    if timestamp<9995: #TODO: CHANGE TO 9975 for official run
        #Do arbitrage
    ...
    elif timestamp>=9995: #TODO: CHANGE TO 9975 for official run
        #SELLLLL EVERYTHING! 
        bound = -2.5
        orders = sell_all_orchids(product, orders, position, acceptable_price, position_limit, bound)
