...
def calculate_moving_avg(traderDataDict, product, price, decay_rate = 0.8):
    ...
    return traderDataDict, moving_avg

    
def trade_moving_avg(product, orders, position, acceptable_price, position_limit, bound):

    if price<=moving_avg-0.05:
        print("BELOW MOVING AVG for ", product)
        buy_amount = position_limit - position
        orders = place_buy_order(orders, product, round(price - bound), buy_amount)
    elif price=>moving_avg+0.05:
        print("ABOVE MOVING AVG for ", product)
        sell_amount = -position_limit + position
        orders = place_sell_order(orders, product, round(price + bound), sell_amount)
    else:
        buy_amount = position_limit - position
        sell_amount = -position_limit + position
        orders = place_buy_order(orders, product, round(price - bound), buy_amount)
